...
## Converted to godot4 by dannygaray60
...
static func beautify_json(json: String, spaces := 0) -> String:
	...
	var error = json_obj.parse(json)

	if error != OK:
		print(
			"JSON Parse Error: ", json_obj.get_error_message(), " in ", json, " at line ", json_obj.get_error_line()
		)
		return json_obj.get_error_message()
